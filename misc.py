...
def create_vid_grid(vids, grid_size):
    array = np.zeros((vids.shape[1],vids.shape[2]*grid_size[0],vids.shape[3]*grid_size[1],vids.shape[4]))
    for i in range(grid_size[0]): # 0 to 6
        for j in range(grid_size[1]): # 0 to 3
            array[:,i*vids.shape[2]:(i+1)*vids.shape[2],j*vids.shape[3]:(j+1)*vids.shape[3]] = vids[j+i*grid_size[1]]
    return (np.clip(np.ceil(array),0,255)).astype('uint8')

def save_video_grid(images, filename, drange=[0,1], grid_size=None):
    images = images.transpose(0,2,3,4,1) # NCDHW
    images = adjust_dynamic_range(images, drange, [0, 255])
    vid=create_vid_grid(images, grid_size)
    skvideo.io.vwrite(filename.replace('.png','.avi'),vid)
