...
from lasagne.layers import InputLayer, Conv3DLayer, DenseLayer, NINLayer
from lasagne.layers import Upscale3DLayer, Pool3DLayer, GlobalPoolLayer, MaxPool3DLayer
...
class PixelNormLayer(lasagne.layers.Layer):
    ...
    def get_output_for(self, v, **kwargs):
        return v / T.sqrt(Tmean(v**2, axis=1, keep_dims=True) + 1.0e-8)
...
class MinibatchStatConcatLayer(lasagne.layers.Layer):
    ...
    def get_output_for(self, input, **kwargs):
        s = list(input.shape)
        vals = self.func(input,axis=0,keep_dims=True)                # per activation, over minibatch dim
        if self.averaging == 'all':                                 # average everything --> 1 value per minibatch
            vals = Tmean(vals,keep_dims=True)
            reps = s; reps[1]=1
            vals = T.tile(vals,reps)
        elif self.averaging == 'spatial':                           # average spatial locations
            if len(s) == 4:
                vals = Tmean(vals,axis=(2,3),keep_dims=True)
            reps = s; reps[1]=1
            vals = T.tile(vals,reps)
            ...
            if len(s) == 4:
                vals = self.func(input,axis=(0,2,3),keep_dims=True)
            reps = s; reps[1]=1
            vals = T.tile(vals,reps)
        elif self.averaging == 'flat':
            vals = self.func(input,keep_dims=True)                   # variance of ALL activations --> 1 value per minibatch
            reps = s; reps[1]=1
            vals = T.tile(vals,reps)
            ...
            vals = Tmean(vals, axis=(2,3,4), keep_dims=True)
...
class GDropLayer(lasagne.layers.Layer):
    ...
    def get_output_for(self, input, deterministic=False, **kwargs):
        ...
        if self.normalize:
            rnd = rnd / T.sqrt(Tmean(rnd ** 2, axis=1, keep_dims=True))
...
class LayerNormLayer(lasagne.layers.Layer):
    ...
    def get_output_for(self, v, **kwargs):
        ...
        v = v - Tmean(v, axis=avg_axes, keep_dims=True) # subtract mean
        v = v * T.inv(T.sqrt(Tmean(T.square(v), axis=avg_axes, keep_dims=True) + self.epsilon)) # divide by stdev
