...
R           = EasyDict(func='networks.DualSWD')             # Options for discriminator network.
G_opt       = EasyDict(beta1=0.0, beta2=0.99, epsilon=1e-8) # Options for generator optimizer.
D_opt       = EasyDict(beta1=0.0, beta2=0.99, epsilon=1e-8) # Options for discriminator optimizer.
G_loss      = EasyDict(func='loss.G_swgan')            # Options for generator loss.
D_loss      = EasyDict(func='loss.D_swgan')          # Options for discriminator loss.
sched       = EasyDict()                                    # Options for train.TrainingSchedule.
grid        = EasyDict(size='1080p', layout='random')       # Options for train.setup_snapshot_image_grid().


# Dataset (choose one).
# desc += '-ucf101';            dataset = EasyDict(tfrecord_dir='ucf101_tfrecords', resolution=128); train.mirror_augment = True
# desc += '-golf';            dataset = EasyDict(tfrecord_dir='golf', resolution=128); train.mirror_augment = True
desc += '-trailerfaces';      dataset = EasyDict(tfrecord_dir='trailerFaces', resolution=256); train.mirror_augment = True
...
# runnable config
# desc += '-preset-v2-1gpu'; num_gpus = 1; sched.minibatch_base = 4; sched.minibatch_dict = {4: 32, 8: 32, 16: 8, 32: 4, 64: 4, 128: 4, 256: 2, 512: 1}; sched.G_lrate_dict = {128: 0.0015}; sched.D_lrate_dict = EasyDict(sched.G_lrate_dict); train.total_kimg = 12000
desc += '-preset-titanxp-1gpu'; num_gpus = 1; sched.minibatch_base = 4; sched.minibatch_dict = {4: 32, 8: 32, 16: 16, 32: 16, 64: 8, 128: 8, 256: 4, 512: 4}; sched.G_lrate_dict = {128: 0.0015}; sched.D_lrate_dict = EasyDict(sched.G_lrate_dict); train.total_kimg = 12000
...
desc += '-fp32'; sched.max_minibatch_per_gpu = {4: 32, 8: 32, 16: 16, 32: 16, 64: 8, 128: 8, 256: 4, 512: 4}
