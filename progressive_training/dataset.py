...
class TFRecordDataset:
    def __init__(self,
        ...
        shuffle_mb=1024,  # Shuffle data within specified window (megabytes), 0 = disable shuffling.
        prefetch_mb=64,  # Amount of data to prefetch (megabytes), 0 = disable prefetching.
        buffer_mb=64,  # Read buffer size (megabytes).
        ...
        max_shape = max(tfr_shapes, key=lambda shape: np.prod(shape)) # 32, 3, 128, 128
        self.resolution = resolution if resolution is not None else max_shape[2] #
        self.resolution_log2 = int(np.log2(self.resolution))
        self.shape = [max_shape[0], max_shape[1], self.resolution, self.resolution] # self.shape = 32,3,128,128
        tfr_lods = [self.resolution_log2 - int(np.log2(shape[2])) for shape in tfr_shapes] # [0,1,2,3,4,5,6]
    ...
    def get_minibatch_tf(self): # => images, labels
        #print('get_minibatch_shape')
        #print(self._tf_iterator.get_next()[0].shape)
        return self._tf_iterator.get_next()#, perm=[1,0,2,3])#,perm=[1,0,2,3])
...
class SyntheticDataset:
    def __init__(self, resolution=1024, num_frames=32, num_channels=3, dtype='uint8', dynamic_range=[0,255], label_size=0, label_dtype='float32'):
        self.resolution         = resolution
        self.resolution_log2    = int(np.log2(resolution))
        self.shape              = [num_frames, num_channels, resolution, resolution]
        self.dtype              = dtype
    ...
    def get_minibatch_tf(self): # => images, labels
        with tf.name_scope('SyntheticDataset'):
            ...
            shape = [self.shape[0], self.shape[1], self.shape[2] // shrink, self.shape[3] // shrink]
