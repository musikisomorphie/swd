...
def setup_snapshot_image_grid(G, training_set,
    ...
    gw = 1; gh = 1
    if size == '1080p':
        gw = np.clip(1920 // G.output_shape[4], 3, 10)
        gh = np.clip(1080 // G.output_shape[3], 2, 10)
    if size == '4k':
        gw = np.clip(3840 // G.output_shape[4], 7, 32)
        gh = np.clip(2160 // G.output_shape[3], 4, 32)

    # Fill in reals and labels.
    reals = np.zeros([gw * gh] + [training_set.shape[1], training_set.shape[0], training_set.shape[2], training_set.shape[3]], dtype=training_set.dtype)
    labels = np.zeros([gw * gh, training_set.label_size], dtype=training_set.label_dtype)
    for idx in range(gw * gh):
        x = idx % gw; y = idx // gw
        while True:
            real, label = training_set.get_minibatch_np(1)
            real = np.transpose(real,[0,2,1,3,4])
