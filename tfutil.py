        ...
        with absolute_name_scope(self.scope):
            ...
            for dev_idx, (dev, grads) in enumerate(dev_grads.items()):
                with tf.name_scope('ApplyGrads%d' % dev_idx), tf.device(dev):
                    ...
                    # update for radon projection
                    for idx, (g,v) in enumerate(grads):
                        if 'proj' in v.name:
                            tmp1 = tf.matmul(tf.transpose(v), g)
                            tmp2 = 0.5 * (tmp1 + tf.transpose(tmp1))
                            g2 = g - tf.matmul(v, tmp2)
                            grads[idx] = (g2, v)
                    stiefel_up = tf.random_normal([128, 128])
                    for g, var in grads: #lib.params_with_name('Discriminator.proj'):
                        if 'proj' in var.name:
                            #print('radon grad {}'.format(name))
                            o_n, _ = tf.qr(var)
                            stiefel_up = stiefel_up + tf.reduce_sum(var.assign(o_n),[0,1])
                    ops.append(stiefel_up)
